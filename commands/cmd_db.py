...
def cli(ctx, user, password):
    try:
        ...
        # if click.confirm("It appears the credentials or wrong, or the database hasn't been configured. Configure now?"):
        #     configure(ctx.obj)
        # os.system('touch configure.json')
        click.echo("""The database needs to be configured. Run: cleancsv db configure""")
        # sys.exit(1)
